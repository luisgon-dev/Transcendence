...
> color *encodes* data, gold is reserved for actions. Tokens are oklch in Tailwind v4
...
- **Color palette**: oklch tokens, theme-flipping via `--t-*` vars. Cool-slate neutrals; off-white/ink text per theme. **Gold accent reserved for actions only** — primary buttons, active state, focus rings. Nothing decorative is gold. **Color encodes data**: a diverging win-rate scale (red↔neutral↔green, centered 50%), tier categorical (S/A/B/C/D), rank categorical (iron→challenger), win/loss semantic.
...
- **Interactive elements**: Solid gold primary buttons (`text-primary-fg`), outline/ghost secondaries. Filters use the `SegmentedControl`/`Select` primitives (Radix-backed) for keyboard a11y. `rounded-control`/`rounded-lg`, not pill clusters.
