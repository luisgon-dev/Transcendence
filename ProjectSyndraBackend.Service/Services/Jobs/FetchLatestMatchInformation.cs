        ...
        foreach (var summoner in summoners)
        {
            ...
            try
            {
                var matchList = await riotGamesApi.MatchV5()
                    .GetMatchIdsByPUUIDAsync(RegionalRoute.AMERICAS, summoner.Puuid, 20, null,
            ...
            foreach (var matchId in totalMatchList)
            {
                try
                {
                    var match = await riotGamesApi.GetMatchDetails(matchId, RegionalRoute.AMERICAS, PlatformRoute.NA1,
                        context, stoppingToken);
                    if (match == null) logger.LogInformation("Match {MatchId} already exists in the database", matchId);
                }
                catch (Exception e)
                {
