...
describe("fetchWithGlobalAnalyticsRegionFallback", () => {
  it("returns the initial result when the region fetch succeeds", async () => {
    const fetcher = vi.fn(async () => ({ ok: true as const, body: ["ok"] }));

    const result = await fetchWithGlobalAnalyticsRegionFallback("NA1", fetcher);
  ...
  it("retries with Global when a non-global region fails", async () => {
    const fetcher = vi
      ...
      .mockImplementation(async (region: string) =>
        region === "ALL" ? { ok: true, source: "global" } : { ok: false, source: "regional" }
      );

    const result = await fetchWithGlobalAnalyticsRegionFallback("EUW1", fetcher);
...
describe("resolveAnalyticsRegionPresentation", () => {
  it("preserves the requested region when no fallback is used", () => {
    expect(resolveAnalyticsRegionPresentation("NA1", "North America", OPTIONS, false)).toEqual({
      effectiveRegion: "NA1",
  ...
  it("switches labels and warning text when global fallback is used", () => {
    expect(resolveAnalyticsRegionPresentation("EUW1", "Europe West", OPTIONS, true)).toEqual({
      effectiveRegion: "ALL",
      effectiveRegionLabel: "Global",
      fallbackMessage: "Europe West data is unavailable right now. Showing Global instead."
    });
  });
});
