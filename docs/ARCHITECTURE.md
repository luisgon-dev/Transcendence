...
- **Build Atlas is precomputed off the request path.** The hourly analytics refresh deduplicates item/rune usage to one row per participant/resource, then persists additive `BuildResourceStat` atoms by patch × region × resource × champion × role. Item and rune index/detail requests roll those indexed atoms up for the selected region and reuse `ChampionRoleTierStat` for exact participant/champion denominators. The raw-match query remains only as a compatibility fallback before the first snapshot exists; normal requests never scan the multi-million-row participant/resource corpus.
- **A precompute cycle is one patch-level commit.** `RefreshPrecomputedAnalyticsJob` calls `RefreshAllAsync`, which executes tabular core → matchups → Build Atlas resources → builds → pro snapshots in that required order inside one database transaction. Build snapshots can read the new core atoms within the transaction, while external readers continue seeing the prior complete snapshot until every phase succeeds. A crash, cancellation, or later-phase exception rolls back all replacements; cache invalidation happens only after the commit. Individual phase methods retain their own transaction when invoked independently by focused tests or maintenance tools.
