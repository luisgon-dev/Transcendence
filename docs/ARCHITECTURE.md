...
- **A champion precompute cycle is one patch-level commit.** `RefreshPrecomputedAnalyticsJob` calls `RefreshAllAsync`, which executes tabular core → matchups → builds → pro snapshots in that required order inside one database transaction. Build snapshots can read the new core atoms within the transaction, while external readers continue seeing the prior complete snapshot until every phase succeeds. The matchup phase partitions the all-champion lane-pair/timeline aggregation into disjoint champion-ID batches, preserving the exact aggregate while bounding individual PostgreSQL commands and using the champion-side covering index. A crash, cancellation, or later-phase exception rolls back all replacements; cache invalidation happens only after the commit. Individual phase methods retain their own transaction when invoked independently by focused tests or maintenance tools. Build Atlas deliberately has its own generation boundary so a slow/failing champion phase cannot block it.
