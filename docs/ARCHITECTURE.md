...
- **Parallel match-detail fetch.** Within a single summoner refresh, the Riot `match-v5` detail fetches run **bounded-parallel** (`MatchService.GetMatchDetailsBatchAsync`, `Jobs:MatchIngestion:MaxParallelMatchFetches` in-flight) so a refresh that finds N new matches overlaps the request latency instead of paying it serially. Strictly two-stage: stage 1 fetches DTOs in parallel (Camille is thread-safe, no DbContext); stage 2 builds the EF entity graphs **sequentially** (the scoped `DbContext` is not thread-safe). Camille still enforces the per-region rate limit, so this overlaps latency without exceeding it.
