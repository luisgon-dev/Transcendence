...
- [ ] **Refresh locks are released by key, not by owner — a stale holder can release a newer holder's lock** `MED` · `medium`
...
- [ ] **Two independent retry mechanisms re-fetch the same failed match, double-spending Riot budget** `MED` · `small`
...
- [ ] **FullHistoryBackfillJob has no per-match exception isolation; one transient Riot error strands the backfill in 'Running' forever** `MED` · `small`
...
- [ ] **Concurrent same-match timeline ingestion races on the snapshot/skill primary keys, burning the retry budget** `MED` · `medium`
...
- [ ] **BFF forwards client-controlled X-Forwarded-For verbatim; backend trusts it and treats private IPs as unlimited** `MED` · `small`
...
- [ ] **Admin BFF same-origin (CSRF) check is spoofable via X-Forwarded-Host** `MED` · `small`
...
- [ ] **FullHistoryBackfillJob has no concurrency guard: overlapping runs for one summoner crash on unique-index collisions and double-spend scarce Riot budget** `MED` · `medium` · ✅ verified
...
- [ ] **RecomputeSeasonAggregatesAsync deletes season aggregates and re-inserts them without a transaction** `MED` · `small`
...
- [ ] **WebAPI controllers bypass Service.Core and access EF/DbContext and repositories directly (query + write logic in the presentation layer)** `HIGH` · `large` · ✅ verified
...
- [ ] **Redundant Match↔Summoner many-to-many join table (`MatchSummoner`) is written on every ingest but never read** `MED` · `medium`
...
- [ ] **Immutable match facts carry hard required FKs to mutable, per-patch static-data tables (ItemVersion / RuneVersion)** `MED` · `large`
...
- [ ] **Nullable columns carry UNIQUE indexes that Postgres does not enforce across NULLs (Match.MatchId, Summoner.Puuid)** `MED` · `medium`
...
- [ ] **Test harness never exercises the real Postgres DDL/migrations and only partially enforces relational constraints** `MED` · `medium`
...
- [ ] **Profile read tracks the full summoner graph and over-fetches unbounded, unused HistoricalRanks** `MED` · `small`
...
- [ ] **Summoner champion-stats and role-breakdown aggregate client-side over the full participant history** `MED` · `medium`
...
- [ ] **Raw tier-list fallback materializes the whole scope's distinct MatchId set into app memory** `MED` · `small`
...
- [ ] **Atom refresh and read-cache invalidation are decoupled, so the "updated N ago" freshness label can overstate the data actually served** `MED` · `small`
...
- [ ] **Uncalibrated tiering defaults (500-game floor + 200-game prior-fit) collapse thin scopes to a uniform B tier list** `MED` · `medium`
...
- [ ] **Match timelines negatively cache an empty gold/XP chart for up to 1h with no invalidation path** `MED` · `small`
...
- [ ] **Empty / zero-sample analytics results inherit the 24h TTL, bounded only by a 2h–24h refresh cadence** `MED` · `small`
...
- [ ] **Analytics warm-writer reconstructs builds/matchups/pro-builds cache keys by hand — silent drift risk vs the readers** `MED` · `small`
...
- [ ] **OpenAPI does not represent C# nullability; generated client types are inverted and never `required`** `MED` · `medium` · ✅ verified
...
- [ ] **Inconsistent error model: admin endpoints return an undocumented `{message,detail}` body instead of ProblemDetails** `MED` · `medium`
...
- [ ] **Many mutation endpoints return anonymous success bodies, producing untyped 200s in the contract** `MED` · `small`
...
- [ ] **Validation-error (ValidationProblemDetails) schema is absent from the contract** `MED` · `small`
...
- [ ] **Profile contract ships a permanent placeholder in `championName`** `MED` · `small`
...
- [ ] **All authentication crypto is untested — hand-rolled PBKDF2 hashing, login verification, JWT signing, and API-key handler are only mocked** `HIGH` · `medium` · ✅ verified
...
- [ ] **No test runs against real PostgreSQL — data layer is validated only on SQLite and the EF InMemory provider** `HIGH` · `large` · ✅ verified
...
- [ ] **Migrations are never applied in any test — schema is built from the model via EnsureCreated, so the 43 migrations' SQL is unverified** `MED` · `medium` · ✅ verified
...
- [ ] **Authorization boundary is never exercised — controller tests bypass [Authorize], API-key scheme, and admin policies via a hand-built principal** `MED` · `large` · ✅ verified
...
- [ ] **Summoner profile abandons server streaming for a full client-side data waterfall** `MED` · `large` · ✅ verified
...
- [ ] **Full DDragon static maps fetched on the client on every profile view** `MED` · `medium`
...
- [ ] **poll-deploy performs no post-deploy health verification and its own pipeline failures are silent — a broken deploy is reported as successful** `MED` · `medium`
...
- [ ] **No metrics-based alerting: Prometheus has no rules/alertmanager and Grafana has no alerting provisioning; in-app alerts cover only ingestion** `MED` · `medium`
  ...
  - **Where:** `config/prometheus/prometheus.yml:1-23; config/grafana/provisioning/ (datasources + dashboards only); Transcendence.Service.Core/Services/Diagnostics/WebhookAlertNotifier.cs:11-21`
- [ ] **Stale/contradictory docs on safety-critical behaviors: CI claims prod 'never auto-migrates' and ARCHITECTURE still describes wud as the deployer** `MED` · `trivial`
...
- [ ] **Web container ships no healthcheck — a hung Next.js process is never detected or restarted** `MED` · `small`
...
- [ ] **Canonical ARCHITECTURE.md documents the retired 'wud' deploy mechanism as the live prod pipeline** `HIGH` · `small`
...
- [ ] **Tier-list interactive elements have no visible keyboard focus indicator (native outline globally removed)** `HIGH` · `small` · ✅ verified
...
- [ ] **Command palette (the flagship nav control) lacks modal and combobox accessibility semantics** `MED` · `medium`
...
- [ ] **Tier-list sticky column headers never actually stick (overflow-x wrapper + header occlusion)** `MED` · `medium`
...
- [ ] **Deep in the ~170-row list there is no persistent way to filter or jump (non-sticky toolbar; xl-only spine)** `MED` · `medium`
...
- [ ] **Command palette is over-decorated and off-brand relative to the restrained system** `MED` · `small`
...
- [ ] **Interactive tab/pill/button controls have no visible keyboard focus indicator (WCAG 2.4.7 failure)** `HIGH` · `small` · ✅ verified
...
- [ ] **Pro Builds detail page is a raw data dump, not a synthesized build — no answer to "which build do I use?"** `MED` · `medium`
...
- [ ] **Confidence pips are undecodable without hover/focus and are a tiny, repeated tab target** `MED` · `small`
...
- [ ] **Mobile: match history is buried below all five sidebar cards (primary content pushed far down)** `HIGH` · `medium` · ✅ verified
...
- [ ] **Ad-hoc low-opacity foreground text tints fall below WCAG AA in the light theme** `MED` · `small`
...
- [ ] **No SEO / discoverability layer — the growth engine every competitor runs on is missing** `HIGH` · `medium` · ✅ verified
...
- [ ] **Analytics is Ranked Solo/Duo ONLY — no ARAM, Arena, Flex or Normal tier lists / champion stats** `HIGH` · `large` · ✅ verified
...
- [ ] **Multi-search (champ-select scouting) is advertised and built in the backend but has no UI** `HIGH` · `medium` · ✅ verified
...
- [ ] **No leaderboards / ladder pages (regional or champion one-trick ladders)** `HIGH` · `medium` · ✅ verified
...
- [ ] **No trend-over-time views: shallow LP history and no per-patch win-rate graphs** `MED` · `medium` · ✅ verified
...
- [ ] **Refresh-lock orchestration duplicated verbatim across SummonersController and ProSummonersController** `MED` · `medium`
...
- [ ] **Service-locator anti-pattern for telemetry inside controllers, with all exceptions swallowed** `LOW` · `small`
...
- [ ] **Committed Prometheus/Grafana observability stack is entirely undocumented despite extensive metric docs** `MED` · `small` · ✅ verified
  ...
  - **Where:** `compose.yml:175-211 & config/grafana/dashboards/ (vs README.md:159-169, docs/DEVELOPMENT.md telemetry sections)`
...
- [ ] **Theme toggle flashes an empty icon and can announce the wrong label before hydration** `LOW` · `trivial`
